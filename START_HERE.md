# 👉 START HERE

## To Play the Game Right Now:

### **1. Find this folder on your computer:**
```
Avalon/
```

### **2. Go into the game folder:**
```
Avalon/game/
```

### **3. Double-click this file:**
```
index.html
```

### **4. Play!**
The game opens in your browser. Click the choice buttons to play.

---

## That's It!

Everything else in this repository is:
- Lore and worldbuilding (in `lore/` folder)
- Novel drafts (in `writing_drafts/` folder)
- Development docs (in `docs/` folder)
- Professional game version (in `choicescript_game/` folder)

**But to just play? Open `game/index.html`**

---

## Folder Guide:

```
Avalon/
│
├── game/
│   └── index.html ← PLAY THIS!
│
├── lore/
│   └── [worldbuilding documents]
│
├── writing_drafts/
│   └── [novel manuscripts]
│
├── docs/
│   └── [guides and plans]
│
└── choicescript_game/
    └── [professional version for publishing]
```

---

## Need Help?

- Can't find the Avalon folder? Search your computer for "Avalon"
- Still stuck? Read `QUICK_START.md` for detailed instructions
- Browser won't open index.html? Right-click → "Open With" → Choose your browser
- Questions? See [COMMUNICATION_CHANNELS.md](COMMUNICATION_CHANNELS.md) for how to reach us

---

## 🤖 NEW: Managing Your AI Workers?

If you're here because you need to manage the autonomous AI workers:

**→ Go to: [AGENT_START_HERE.md](AGENT_START_HERE.md)**

This covers:
- Verifying your closed PRs worked ✅
- Using the Agent Management System
- Checking system health (30 seconds)
- Activating the AI workers

---

**The game is ready to play. Just find `game/index.html` and open it!**
